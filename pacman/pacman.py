...
def readCommand( argv ):
    ...
    parser.add_option('-d', '--dump', dest='dump',
                      help=default('the DUMP_FILE to which to dump the shield to'),
                      metavar='DUMP_FILE', default='')
    parser.add_option('-o', '--open', dest='open',
                      help=default('the OPEN_FILE from which to load the shield'),
...
def runGames(layout, pacman, ghosts, display, numGames, record, numTraining = 0, numGhostTraining = 0, withoutShield = 0, catchExceptions=False, timeout=60, symX=False, symY=False ):
    import __main__
    ...
    file_name_scores = "outputs/" + "scores_" + str(layout.name) + "_w" + str(withoutShield) + "_n" + str(numGames) + "_x" + str(numTraining) + "_y" + str(numGhostTraining) + ".txt"
    file_name_wins = "outputs/" + "wins_" + str(layout.name) + "_w" + str(withoutShield) + "_n" + str(numGames) + "_x" + str(numTraining) + "_y" + str(numGhostTraining) + ".txt"
    file_scores = open(file_name_scores, "w+")
    file_wins = open(file_name_wins, "w+")
    ...
    for i in range( numGames ):
        ...
        if (i+1)%10==0:
            ...
            file_scores.write(str(sum(scores) / float(len(scores))) + "\n")
            print('Scores:       ', ', '.join([str(score) for score in scores]))
            print('Win Rate:      %d/%d (%.2f)' % (wins.count(True), len(wins), winRate))
            file_wins.write(str(winRate) + "\n")
            print("------------------------------------------")
